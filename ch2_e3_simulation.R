...
input$sorQp1 = .1
input$sorPp1 = .8
input$sorQon1 = c(1,30)
# input$sorQon2 = c(80,110)
# input$sorPon1 = c(40,70)
# input$sorPon2 = c(80,110)
input$sordur = 100
input$sorpd1 = 0.1
input$sorpd2 = 0.02
input$V = 1
...
sor = matrix(nrow = input$sordur, ncol = 29)
...
for (i in 1:nrow(sor)){
    if (i == 1){
        ...
        # p2
        sor[i,'p2'] = input$V * ((input$w1 * sor[i,'pA1Q']) + (input$w2 * sor[i,'pA2Q']))
...
png("ch2e3_plot1.png", width = 800, height = 400)

text1 = 20
text2 = 18

ggplot(data=as.data.frame(sor), aes(x = time, y=pA1Q, group=1, color="Q" )) + ylim(0,1) +
    geom_line(size = 1) + geom_line(aes(y=pA2Q, colour = 'Q'), size =1, linetype = 'longdash') + theme_classic() + 
    geom_line(aes(y=p1Q, colour = 'p1'), size =2, linetype = 'longdash') +
    geom_line(aes(y=p2, colour = 'p2'), size =2, linetype = 'longdash') +
    ...
    ggtitle("A. SOP Simulation, p1 = 0.1") + 

...
for (i in 1:nrow(sor)){
    if (i == 1){
        ...
        sor[i,'pA1Qloss'] = sor[i,'pA1Q'] * sor[i-1,'pd1'] # how much decays to A2
        ...
        sor[i,'pA2Qloss'] = sor[i,'pA2Q'] * sor[i-1,'pd2'] # how much decays A2->I
        ...
        sor[i,'pd1'] = sor[i-1,'pd1'] + (sor[i,'pA1P'] / input$C1)
        sor[i,'pd1'] = sor[i-1,'pd2'] + (sor[i,'pA2P'] / input$C2)
...
png("rr_is9_C_plot.png", width = 800, height = 400)
...
ggplot(data=as.data.frame(sor), aes(x = time, y=pA1P, group=1, color="P" )) +
    ...
    ggtitle("RR ISI9 Simulation with Update") + 
